...
const ProgressCard: React.FC<ProgressCardProps> = ({ currentValue, targetValue, label }) => {
    ...
    const animatedValue = useAnimatedCounter(currentValue, 0, 0.5);
    ...
    return (
        <div className="bg-white flex px-5 py-4 pb-8 gap-4 shadow-card-shadow border-1.7 border-card-gray rounded-lg text-sm">
            <div className="flex flex-col gap-2">
                ...
                <h1 className="ml-3 text-4xl font-bold text-gray-800">{Math.floor(animatedValue)} <span className="text-[#C1C1C1] text-2xl">/ {targetValue}</span></h1>
            </div>
