...
class TimesheetRepository:
    ...
    def get_current_timesheet(self, username: str):
        ...
        if username is None:
            return None
        try:
            timesheet_data = self.db.timesheets.find_one(
                {"username": username, "status": TimesheetStatus.NOT_SUBMITTED.value})
            if timesheet_data:
                return timesheet_data
            return None
        except PyMongoError as e:  # pragma: no cover
        ...
        try:
            timesheet_data = self.db.timesheets.find({"username": username,
                                                      "$or": [
                                                          ...
                                                          {"$and": [{"year": start_year}, {"year": {"$lt": end_year}}],
                                                           "month": {"$gte": start_month}},
                                                          {"$and": [{"year": end_year}, {"year": {"$gt": start_year}}],
                                                           "month": {"$lte": end_month}},
                                                          {"$and": [{"year": end_year}, {"year": start_year},
                                                                    {"month": {"$gte": start_month}},
                                                                    {"month": {"$lte": end_month}}]}
                                                      ]})
    ...
    def create_timesheet_by_dict(self, timesheet_data: dict):
        ...
        try:
            if self.get_timesheet(timesheet_data['username'], timesheet_data['month'],
                                  timesheet_data['year']) is not None:
